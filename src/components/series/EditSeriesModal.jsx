...
const EditSeriesModal = ({
    ...
    useEffect(() => {
        if (isOpen && series) {
            // Hiển thị thumbnail hiện tại của series
            if (series.serie_thumbnail) {
                setImagePreview(series.serie_thumbnail);
            } else {
                setImagePreview(null);
            }

            // Reset state liên quan đến file mới
            setCoverImage(null);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        // Truyền thông tin thumbnail mới (nếu có) cho hàm onSubmit
        onSubmit(coverImage);
    };
            ...
            <div className="relative bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg md:max-w-xl w-full" onClick={e => e.stopPropagation()}>
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-3 sm:px-6">
                    <div className="flex items-center justify-between">
                        <h3 className="text-lg leading-6 font-medium text-white">Chỉnh Sửa Series</h3>
                        <button
                            onClick={onClose}
                                    ...
                                    <div className="w-full h-48 bg-gray-100 rounded-md overflow-hidden border border-dashed border-gray-300 flex items-center justify-center">
                                        {imagePreview ? (
                                            <div className="relative w-full h-full">
                                                <img
                                                    src={coverImage ? imagePreview : series.serie_thumbnail}
                                                    alt="Preview"
                                                ...
                                                <div className="absolute inset-0 bg-black bg-opacity-40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                                                    <label
                                                        ...
                                                        className="cursor-pointer text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-2 rounded shadow-sm text-sm font-medium mx-2"
                                                    >
                                                        Thay thế ảnh
                                                        <input
                                                            id="file-upload-edit"
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => {
                                                            // Xóa ảnh đã chọn và quay lại hiển thị ảnh ban đầu
                                                            setCoverImage(null);
                                                            setImagePreview(series.serie_thumbnail);
                                                            if (fileInputRef.current) fileInputRef.current.value = '';
                                                        }}
                                                        className="text-white bg-red-600 hover:bg-red-700 px-3 py-2 rounded shadow-sm text-sm font-medium mx-2"
                                                    >
                                                        Hủy thay đổi
                                                    </button>
                                                </div>
